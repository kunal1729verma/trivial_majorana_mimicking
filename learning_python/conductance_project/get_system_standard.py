...
def get_onsite_term(site, prof_fns : ProfileFunctions):   # signifies that the second input is an object of type ProfileFunctions.
    ...
    # (x,) = site.pos + 1
    x = site
    H_ii = (prof_fns.t_n(x + 1/2) + prof_fns.t_n(x - 1/2) - prof_fns.μ_n(x) + prof_fns.γ_n(x)) * np.kron(s_z, s_0) + prof_fns.Δz_n(x) * np.kron(s_0, s_x) + prof_fns.Δ_n(x) * np.kron(s_x, s_0)
...
def get_left_lead_onsite(prof_fns : ProfileFunctions):
    return (2 * prof_fns.t_1 - prof_fns.μ_L) * np.kron(s_z, s_0) + prof_fns.Δz * np.kron(s_0, s_x)
